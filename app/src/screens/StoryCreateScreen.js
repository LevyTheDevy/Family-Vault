...
export default function StoryCreateScreen({ navigation }) {
  ...
  const handlePost = async () => {
    if (!image) return Alert.alert('No photo selected');
    setUploading(true);
    try {
      await uploadStory(image, duration, caption.trim());
      navigation.goBack();
    } catch {
      Alert.alert('Failed', 'Could not post daily.');
    } finally {
      setUploading(false);
    }
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.bg }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        {image ? (
          <View style={styles.previewWrap}>
            ...
            <View style={styles.previewActions}>
              <TouchableOpacity style={styles.previewBtn} onPress={takePic}>
                <Feather name="camera" size={14} color="#fff" />
                <Text style={styles.previewBtnText}>Retake</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.previewBtn} onPress={pick}>
          ...
          <View style={[styles.placeholder, { borderColor: colors.border, backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.placeholderText, { color: colors.textSub }]}>Select a photo for your daily</Text>
            <View style={styles.mediaRow}>
              <TouchableOpacity style={[styles.mediaBtn, { borderColor: colors.border, backgroundColor: colors.surface }]} onPress={takePic}>
                <Feather name="camera" size={18} color={colors.text} />
                <Text style={[styles.mediaBtnText, { color: colors.text }]}>Camera</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.mediaBtn, { borderColor: colors.border, backgroundColor: colors.surface }]} onPress={pick}>
        ...
        <View style={[styles.footer, { borderTopColor: colors.border, paddingBottom: insets.bottom + 16 }]}>
          <TouchableOpacity
            style={[styles.postBtn, { backgroundColor: colors.accent }, (!image || uploading) && styles.postBtnDisabled]}
            onPress={handlePost}
...
const styles = StyleSheet.create({
  ...
  mediaRow: { flexDirection: 'row', gap: 10, paddingHorizontal: 24 },
  ...
  footer: { padding: 16, borderTopWidth: 1, marginTop: 'auto' },
  postBtn: { borderRadius: 8, paddingVertical: 14, alignItems: 'center' },
